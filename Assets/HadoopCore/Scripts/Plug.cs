...
namespace HadoopCore.Scripts
{
    ...
    public class ShakeConfig
    {
        ...
        [Header("Shake Algorithm")]
        public ShakeAlgorithm algorithm = ShakeAlgorithm.RandomNoise;
        ...
        [Header("Curves")]
        [Tooltip("强度曲线：输入 level01(0..1)，输出强度倍率(0..1)")]
    ...
    public class Plug : MonoBehaviour, IPointerClickHandler
    {
        ...
        private void Shaking()
        {
            ...
            shakeConfig.shakeCoroutine = StartCoroutine(ShakeRoutine(posAmp, rotAmp, shakeConfig.duration, shakeConfig.frequencyHz, shakeConfig.algorithm));
        }
            ...
            ShakeConfig.ShakeAlgorithm algo)
        {
            ...
            while (elapsed < duration)
            {
                ...
                if (algo == ShakeConfig.ShakeAlgorithm.RandomNoise)
                {
                    ...
                    shakeConfig.shakePivot.localRotation = shakeConfig.baseLocalRot * Quaternion.Euler(0f, 0f, rotOffset);
                }
                else // DampedSine
                {
                    // 规律振荡 + 包络衰减：更像"弹簧回弹"
